...
Implementation status (2026-09-05): the model-independent media foundation is implemented. A Rust-owned `opencut.media-capability-catalog.v1` exposes stable subject-tracking, audio-cleanup, stem-separation, and VAD task IDs with task-specific bounded option contracts plus typed provider-job, source-identity, provenance, CPU-fallback, and deterministic-cache requirements through both `opencut_capabilities` and `opencut_get_media_capability_catalog`. Capability readiness and legacy execution blockers are projected from the same Rust policy. Every task truthfully reports `model-selection-required`, `canExecute: false`, provider execution forbidden, and zero cost incurred. The legacy `opencut_clean_audio` and `opencut_track_subject` command paths are hard-gated with the same typed blocker even if command environment variables are configured. Provider histories enforce submitted, started, progress, optional retry with attempt transitions, and completed states. This status does not approve, download, bundle, probe by inference, or claim readiness for any local model; provider execution and production evidence remain blocked on the owner decisions recorded in section 26.
...
Implementation status (2026-09-05): versioned Rust contracts now validate and canonically persist externally produced tracking and VAD analysis objects with exact source identity, bounded task-specific semantic inputs including requested tracking coverage, source-time samples/ranges, corrections, attachment bindings, provider/adapter/model/license/runtime/device identity, enforced and chronologically ordered start/progress/retry/completion lifecycle history, cost state, output-artifact inventory, semantic input hashes, cache identities, and object content hashes. Before persistence, Rust binds tracking coverage and subject/sample counts plus VAD result channels, requested ranges, minimum durations, and correction ranges back to the hashed semantic request; correction targets must resolve within the durable object. Rust owns create/replay/conflict resolution. It also validates and deterministically plans typed ordered clip/track/master audio graphs and VAD-derived ducking envelopes, merging attack/release-expanded overlaps under an explicit policy. These are reference-independent contracts and plans only: no cleanup, stem, VAD, or tracker model is selected; no provider job is executed; no graph is applied by the editor or renderer; no A/B, aligned-stem, preview/export-parity, real-video, or receipt-level QC evidence is claimed. Public stdio coverage is in `packages/mcp-server/src/media-foundation-stdio.test.ts`.
