...
Implementation status (2026-09-04, issue #26): `opencut.time-map.v1` and `set_time_map` implement the general durable model, Rust-owned boundary/trim/split/query/tracker/audio semantics, independent dynamic pitch policy, source-time readback, and explicit nearest-frame fallback. `startTime` repositions a mapped clip without changing its map; `timeMapRange` trims to any clip-local `[start,end]` of the map, with Rust slicing and rebasing across reverse and hold boundaries; `duration` alone crops only the right timeline edge and must equal `end-start` when a range is given; source trims remain fixed; keyframes remain timeline-anchored and are clamped, not rebased, by a range trim; and split slices/rebases both maps. Windowed audio preview slices the map to the visible window instead of shifting source trims, separated audio carries the map, and native drag-resize refuses mapped clips. Real Chrome evidence compares ramp, freeze, and reverse preview frames with export, compares the pitch-preserved preview audio with export, reads back the rebased source times after a two-edge `timeMapRange` trim, and measures the exported and previewed fundamental of the 440 Hz fixture tone within 35 Hz of 440 Hz through the early ramp, late ramp, and reverse windows where unpreserved audio would read about 352 Hz, 528 Hz, and 220 Hz. The named Montage treatment remains intentionally unspecified and unimplemented until the required owner reference and numeric tolerance are recorded.
