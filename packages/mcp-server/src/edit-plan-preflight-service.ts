...
export class EditPlanPreflightService {
	...
	async preflight(input: PreflightEditPlanInput): Promise<EditPlanPreflightServiceResult> {
		const parsed = preflightEditPlanInputSchema.parse(input);
		const requestFingerprint = preflightEditPlanRequestFingerprint(parsed);
		const claim = await this.store.claim(parsed.preflightId, requestFingerprint);
		...
		const published = await this.queryBrowserReceipt(parsed, requestFingerprint);
		if (!published) {
			throw new Error("browser returned a preflight without a durable receipt");
		}
		if (canonicalEditPlanJson(published.result) !== canonicalEditPlanJson(terminal)) {
			throw new Error("browser response differs from its durable preflight receipt");
		}
		const receipt = buildReceipt(
			parsed,
			requestFingerprint,
	...
	async verifiedApplication(input: {
		...
		evaluation: Extract<BrowserEditPlanPreflightResponse, { status: "validated" }>["evaluation"];
	}> {
		...
		if (
			...
			throw new Error("apply request does not match the verified preflight receipt");
		}
		const terminal = browserEditPlanPreflightResponseSchema.parse(
		...
		if (
			...
			canonicalEditPlanJson(source.connectionIdentity) !==
				canonicalEditPlanJson({
					...input.expectedConnectionIdentity,
					bridgeProtocolVersion: 2,
				})
		) {
			throw new Error("verified preflight receipt source binding is inconsistent");
		}
...
function parseBrowserPayload(
	...
	if (record.bridgeProtocolVersion !== 2) {
		throw new Error("browser preflight response has invalid bridge protocol metadata");
	}
	...
	if (expectedRequestIdentity) {
		if (
			...
			throw new Error("browser preflight response has invalid request identity metadata");
		}
	} else if (Object.hasOwn(record, "requestConnectionIdentity")) {
		throw new Error("untargeted browser receipt query returned request identity metadata");
	}
	const {
...
function buildReceipt(
	...
	const evaluation = terminal.status === "validated" ? terminal.evaluation : null;
	return {
		schemaVersion: 1,
...
function replayResult(receipt: EditPlanPreflightReceipt): EditPlanPreflightServiceResult {
	return {
		schemaVersion: EDIT_PLAN_PREFLIGHT_SCHEMA,
		disposition: "replayed",
		receiptId: receipt.receiptId,
		result: browserEditPlanPreflightResponseSchema.parse(receipt.terminalResult),
	};
}
...
function validateEvidence(
	...
	if (canonicalEditPlanJson(evaluation.source) !== canonicalEditPlanJson(source)) {
		throw new Error("evaluation source does not match the requested source binding");
	}
	...
	if (
		canonicalEditPlanSha256(evaluation.before) !== input.expectedProjectContentHash ||
		evaluation.before.project.activeSceneId !== input.sceneId
	) {
		throw new Error("evaluation before snapshot does not match the source project");
	}
	if (
		canonicalEditPlanSha256(evaluation.predictedAfter) !==
			evaluation.predictedProjectHash ||
		evaluation.predictedAfter.project.activeSceneId !== input.sceneId
	) {
	...
	if (
		...
		throw new Error("evaluation changed-object evidence does not match snapshots");
	}
	if (
	...
	if (
		...
		throw new Error("evaluation result contains inconsistent summary or cost evidence");
	}
