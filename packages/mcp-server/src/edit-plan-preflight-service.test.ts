...
describe("edit-plan preflight service", () => {
	test("persists and exactly replays a terminal rejection without redispatch", async () => {
		const directory = await mkdtemp(join(tmpdir(), "opencut-preflight-service-"));
	...
	test("rejects out-of-range and semantically false operation attribution", async () => {
		for (const mutation of [
			...
			const response = validatedResponse(request);
			mutation(response);
			response.evaluation.planDiffHash = evaluationDiffHash(response.evaluation);
			const service = new EditPlanPreflightService(
				new DurableTestBrowser(() => response),
				store,
			);
			await expect(service.preflight(request)).rejects.toThrow(/operation|timing/);
			store.close();
		}
	});
...
class DurableTestBrowser {
	...
	async request(method: string, params: unknown): Promise<unknown> {
		...
		if (method !== "preflight_edit_plan") throw new Error(`unexpected ${method}`);
		const request = params as PreflightEditPlanInput;
		this.evaluations += 1;
...
function validatedResponse(request: ReturnType<typeof input>) {
	...
	predictedAfter.project.scenes[0]!.tracks[0]!.elements = [];
	...
	return {
		...
		noMutationProof: { unchanged: true as const, before: observation, after: observation },
	};
}
