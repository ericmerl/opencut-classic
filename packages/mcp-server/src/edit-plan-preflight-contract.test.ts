...
describe("edit-plan preflight contract", () => {
	...
	test("accepts duration-clamp IDs only in resolved operations", () => {
		...
		for (const operation of operations) {
			expect(resolvedEditOperationSchema.safeParse(operation).success).toBe(true);
			expect(
				preflightEditPlanInputSchema.safeParse({
					...request(),
	...
	test("accepts only the full project-content v1 projection", () => {
		const projection = {
		...
		expect(changes).toEqual(
			expect.arrayContaining([
				expect.objectContaining({
					...
					fieldPath:
						"@scene:scene-main/track:track-main/element:compound-1",
