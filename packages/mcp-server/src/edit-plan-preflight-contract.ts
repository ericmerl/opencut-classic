...
type CanonicalElement = z.infer<ReturnType<typeof canonicalElementVariantSchema>>;
...
function canonicalElementVariantSchema(trackSchema: z.ZodType<CanonicalTrack>) {
	...
	return z.discriminatedUnion("type", [
		z.object({ ...canonicalElementCommonShape, type: z.literal("audio"), sourceType: z.string().min(1), mediaId: nullableStringSchema, sourceUrl: nullableStringSchema, retime: canonicalValueSchema, audioReplacement: attachment }).strict(),
		z.object({ ...canonicalElementCommonShape, type: z.literal("video"), ...media, ...visual, isSourceAudioEnabled: nullableBooleanSchema, retime: canonicalValueSchema, masks: z.array(canonicalMaskSchema), matte: attachment, audioReplacement: attachment }).strict(),
		z.object({ ...canonicalElementCommonShape, type: z.literal("image"), ...media, ...visual, masks: z.array(canonicalMaskSchema) }).strict(),
		z.object({ ...canonicalElementCommonShape, type: z.literal("text"), ...visual }).strict(),
		z.object({ ...canonicalElementCommonShape, type: z.literal("sticker"), ...visual, stickerId: z.string().min(1), intrinsicWidth: nullableNumberSchema, intrinsicHeight: nullableNumberSchema }).strict(),
		z.object({ ...canonicalElementCommonShape, type: z.literal("graphic"), ...visual, definitionId: z.string().min(1), masks: z.array(canonicalMaskSchema) }).strict(),
		z.object({ ...canonicalElementCommonShape, type: z.literal("effect"), effectType: z.string().min(1) }).strict(),
		z.object({ ...canonicalElementCommonShape, type: z.literal("compound"), hidden: nullableBooleanSchema, tracks: z.array(trackSchema) }).strict(),
	]);
}
...
const canonicalMediaSourceSchema = z.discriminatedUnion("kind", [
	z.object({ kind: z.literal("local"), contentHash: immutableHashSchema.nullable() }).strict(),
	z.object({
				...
				scenes: z.array(
					z
						.object({
							...
							isMain: z.boolean(),
							bookmarks: z.array(
								z.object({ order: z.number().int().nonnegative(), time: z.number().int().nonnegative(), duration: z.number().int().nonnegative().nullable(), note: nullableStringSchema, color: nullableStringSchema }).strict(),
							),
	...
	.superRefine((value, context) => {
		for (const key of ["before", "after"] as const) {
			try {
				...
				context.addIssue({ code: "custom", path: [key], message: String(error), input: value[key] });
			}
		}
	});
...
export const editPlanEvaluationResponseSchema = z.discriminatedUnion("status", [
	z.object({ status: z.literal("validated"), result: editPlanEvaluationSchema }).strict(),
	z
		.object({
...
const editPlanErrorSchema = editPlanEvaluationResponseSchema.options[1].shape.error;
...
export function canonicalEditPlanSha256(value: unknown): string {
	return createHash("sha256").update(canonicalEditPlanJson(value)).digest("hex");
}
...
function flattenProjectObjects(
	...
	const add = (
		...
		const instances = objects.get(key) ?? new Map<string, OwnedCanonicalObject>();
		...
		for (const track of tracks) {
			...
			for (const element of track.elements) {
				...
				if (!Number.isSafeInteger(end)) {
					throw new Error("project summary duration exceeds the safe integer range");
				}
...
function parseCanonicalValue(
	...
	if (typeof value === "number") {
		if (!Number.isFinite(value) || Math.abs(value) > Number.MAX_SAFE_INTEGER) {
			throw new Error(`number exceeds the finite safe numeric range at ${formatPath(path)}`);
		}
	...
	for (const key of Object.keys(object).sort()) {
		...
		if (containsUnpairedSurrogate(key)) {
			throw new Error(`unpaired UTF-16 surrogate at ${formatPath([...path, key])}`);
		}
