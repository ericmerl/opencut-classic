...
- `opencut_preflight_edit_plan`, evaluating a complete v2 plan against an explicit active or non-active saved scene without mutating editor UI state and returning an immutable receipt
...
- `opencut_queue_export`, persisting the same render specification as a job that survives MCP restarts and runs automatically when an authenticated editor worker connects
...
- `opencut_create_review_annotation`, `opencut_get_review_annotation`, `opencut_list_review_annotations`, and `opencut_update_review_annotation_status`, creating immutable append-versioned human or automated findings against a hash-locked preview frame, half-open preview range, or export, with normalized regions, severity, status, reviewer notes, detector/model provenance, resolution links, optional project bookmarks, and bounded stable pagination
- `opencut_record_watermark_inspection` and `opencut_get_watermark_inspection`, persisting the exact opening, middle, ending, and four-corner sampling policy, independently verified preview/render provenance, final-export-byte evidence, and the human or automated reviewer identity
- `opencut_sign_off_export_review`, requiring complete clean watermark evidence from the final exported bytes, explicit human review, and no unresolved blocking findings before an immutable final sign-off can be recorded
- `opencut_evaluate_export_qc`, evaluating an export receipt against a versioned policy with numeric thresholds and pass/warn/fail findings for container, codec, dimensions, frame rate, duration, decode, streams, hashes, black/frozen video, caption clipping/safe zones, colour, audio loudness/peak/clipping/channels/sample rate/silence/sync, inspection, and platform limits
- `opencut_get_export_qc`, reading a durable QC report after a restart while re-verifying every referenced artifact hash
- `opencut_create_delivery_package`, creating a deterministic collision-safe directory with master, clean and burned-in variants, subtitle sidecars, exact cover frames, inspection evidence, source receipts, QC reports, and a hash/provenance manifest
- `opencut_verify_delivery_package`, failing if a packaged manifest or any declared file is missing or changed
...
Exports never overwrite an existing file. FFmpeg and FFprobe are preflighted before rendering. The renderer loads the fresh persisted project readback bound to the verified save receipt, derives a cloned render tree with the overlay, and leaves the project and editor interaction state unchanged. MP4 currently resolves to AVC and WebM to VP9; incompatible explicit codec requests fail before rendering. A completed export is accepted only after its container and stream metadata match the request, the complete file decodes without an FFmpeg error, and opening, middle, and ending full-frame PNG samples have been extracted and hashed. The immutable core receipt and separately updateable watermark-inspection record are stored under `OPENCUT_RECEIPT_DIR`. Retrying the same operation ID and identical arguments after an MCP restart verifies the output size and SHA-256 before returning the durable result. Batch summaries expose a durable manifest path and retain every variant's requested input, terminal result, and error independently, including partial failures.

Structured review evidence is stored in append-only SQLite under `OPENCUT_RECEIPT_DIR/review-evidence`. Annotation status or resolution changes append a new version and preserve the complete predecessor chain. Reads verify record checksums and referenced artifact hashes, and fail closed when evidence has changed. Review mutations cross the same durable operation boundary as editor mutations, so exact retries replay across MCP restarts while changed input is rejected. Automated detector output remains distinct from human findings and never satisfies the human final-review requirement.

QC policies are explicit version-1 values. Every enabled check emits a structured finding with its outcome, threshold and measurement plus a timestamp, canvas region, and hash-locked evidence paths when applicable. Caption findings use geometry measured by the same layout implementation as the renderer. Delivery packaging accepts only hash-valid exports and QC reports; a clean variant must have been rendered with captions off and a burned-in variant must retain captions. The package directory name is derived from the package name and operation ID, the manifest records source project/save/renderer provenance, and both exact retry and restart verification rehash every declared file.
