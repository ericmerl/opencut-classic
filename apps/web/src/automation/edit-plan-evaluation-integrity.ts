...
export async function verifyEditPlanEvaluationIntegrity({
	...
	const capabilityHash = await sha256(
		canonicalSerialize({
			editPlanReady: evaluation.requirements.editPlanReady,
			providerExecution: evaluation.requirements.providerExecution,
			cost: evaluation.requirements.cost,
		}),
	);
	if (capabilityHash !== evaluation.requirements.hash) {
		return "native capability snapshot hash is invalid";
	}
	...
	const changedObjects = diffProjectSnapshots({
		...
		after: evaluation.predictedAfter,
	});
	if (!equal({ left: changedObjects, right: evaluation.changedObjects })) {
		return "native changed-object evidence does not match prediction";
	}
...
function toCanonicalValue(value: unknown): CanonicalValue {
	...
	if (isPlainObject(value)) {
		return Object.fromEntries(
			Object.entries(value).map(([key, child]) => [key, toCanonicalValue(child)]),
		);
	}
...
function compareOrdinal({ left, right }: { left: string; right: string }): number {
