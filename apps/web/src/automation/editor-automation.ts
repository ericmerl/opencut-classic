...
import type {
	...
	AutomationVerifyEditPlanPreflightSourceResult,
...
export class EditorAutomation {
	...
	verifyEditPlanPreflightSource(
		request: AutomationVerifyEditPlanPreflightSourceRequest,
	): Promise<AutomationVerifyEditPlanPreflightSourceResult> {
		return this.enqueue(async () => {
			const result = await captureEditPlanPreflightSource({
				editor: this.editor,
				request,
			...
			return result.status === "captured"
				? { status: "verified", observation: result.observation }
				: result;
		});
	}
