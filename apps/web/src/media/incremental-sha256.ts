export const SHA256_IMPLEMENTATION = "opencut-typescript-streaming" as const;
export const SHA256_IMPLEMENTATION_VERSION = 1 as const;
export const DEFAULT_SHA256_CHUNK_BYTES = 4 * 1024 * 1024;
...
export class IncrementalSha256 {
	...
	update(bytes: Uint8Array): this {
		...
		if (!Number.isSafeInteger(this.byteLength)) {
			throw new Error(
				"SHA-256 input length exceeds JavaScript's safe integer range",
			);
		}
		let offset = 0;
	...
	digestHex(): string {
		...
		const bitLengthLow = (this.byteLength << 3) >>> 0;
		writeUint32(finalBlock, finalLength - 8, bitLengthHigh);
		writeUint32(finalBlock, finalLength - 4, bitLengthLow);
		for (let offset = 0; offset < finalLength; offset += 64) {
			this.processBlock(finalBlock, offset);
		}
...
export async function hashBlobIncrementally(
	...
	const chunkBytes = options.chunkBytes ?? DEFAULT_SHA256_CHUNK_BYTES;
	if (!Number.isSafeInteger(chunkBytes) || chunkBytes <= 0) {
		throw new Error("SHA-256 chunkBytes must be a positive safe integer");
	}
	...
	while (processed < blob.size) {
		throwIfAborted(options.signal);
		const end = Math.min(processed + chunkBytes, blob.size);
		const bytes = new Uint8Array(
			await blob.slice(processed, end).arrayBuffer(),
		);
		throwIfAborted(options.signal);
		hash.update(bytes);
		processed = end;
		options.onProgress?.(processed, blob.size);
	}
...
function throwIfAborted(signal: AbortSignal | undefined): void {
	...
	throw new DOMException(
		signal.reason?.toString() ?? "Hashing aborted",
		"AbortError",
	);
}
